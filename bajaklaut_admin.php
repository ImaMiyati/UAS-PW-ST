...
$label = 'Data Bajak Laut';
$nama_tabel = 'karakter';
$tipe = 'bajak laut';

$search = isset($_GET['cari']) ? trim($_GET['cari']) : '';
$min_bounty = isset($_GET['min_bounty']) ? trim($_GET['min_bounty']) : '';
$max_bounty = isset($_GET['max_bounty']) ? trim($_GET['max_bounty']) : '';
...
<body>
    <div class="top-bar">
        <div class="judul-tambah">
            <h1><?= $label ?></h1>
            <div>
                <a class="tambah-btn" href="tambahkarakter.php?tipe=<?= urlencode($tipe) ?>">+ Tambah Karakter</a>
            </div>
        </div>
        <form class="search-form" method="GET" action="">
            <input type="text" name="cari" placeholder="Nama atau Kode Karakter" value="<?= htmlspecialchars($search) ?>">
            <input type="text" name="min_bounty" placeholder="Min Bounty (cth: 10000000)" value="<?= htmlspecialchars($min_bounty) ?>">
            <input type="text" name="max_bounty" placeholder="Max Bounty (cth: 50000000)" value="<?= htmlspecialchars($max_bounty) ?>">
    ...
    <table>
        ...
        <tbody>
            ...
            while ($row = mysqli_fetch_assoc($query)):
                $bounty_clean = preg_replace('/\D/', '', strip_tags($row['bounty']));
                if (($min_bounty !== '' && $bounty_clean < $min_bounty) ||
                    ($max_bounty !== '' && $bounty_clean > $max_bounty)) {
                    continue;
                }
                $found = true;
                $id = $row['id'];
            ...
            <tr>
                ...
                <td><?= htmlspecialchars(str_replace('<br>', ' ', $row['bounty'])) ?></td>
                <td style="text-align: center;">
                    <?php if (!empty($row['foto'])): ?>
                        <img src="tampil_foto.php?id=<?= $id ?>" alt="Foto <?= htmlspecialchars($row['nama_karakter']) ?>" style="width: 100px; height: 100px; object-fit: contain; border: 1px solid #ccc;" />
                    <?php else: ?>
  ...
  <a class="download-btn" href="cetak_pdf.php?id=<?= $row['kode_karakter'] ?>&download=1" download>Download PDF</a>
            ...
            <?php if (!$found): ?>
            <tr>
                <td colspan="8" style="text-align: center;">Tidak ada karakter ditemukan.</td>
            </tr>
            <?php endif; ?>
        </tbody>
    </table>
